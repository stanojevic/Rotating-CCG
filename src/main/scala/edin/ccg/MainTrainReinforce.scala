...
import edin.ccg.parsing.RevealingModelReinforce
import edin.nn.DynetSetup
import edin.general.{Global, TrainingController}

object MainTrainReinforce {

  case class CMDargs(
                      original_model_dir          : String    = null,
                      reinforce_model_dir         : String    = null,
                      reinforce_hyper_params_file : String    = null,
  ...
  def main(args:Array[String]) : Unit = {
    val parser = new scopt.OptionParser[CMDargs](PROGRAM_NAME) {
      ...
      opt[ String   ]( "reinforce_model_dir"         ).action((x,c) => c.copy( reinforce_model_dir         = x  )).required()
      opt[ String   ]( "reinforce_hyper_params_file" ).action((x,c) => c.copy( reinforce_hyper_params_file = x  )).required()
    ...
    parser.parse(args, CMDargs()) match {
      case Some(cmd_args) =>
        ...
        val model = new RevealingModelReinforce( originalModelDir  = cmd_args.original_model_dir )
        ...
        new TrainingController(
          ...
          modelDir         = cmd_args.reinforce_model_dir,  // String,
          hyperFile        = cmd_args.reinforce_hyper_params_file,  // String,
