...
// Taken from https://github.com/ronniedong/Expect-for-Java/blob/master/Expect.java
// Modified to not depend on log4j which means that there will be no logging info printed out
...
public class Expect {
    ...
    public Expect(InputStream input, OutputStream output) {
        try {
            ...
            System.err.println("Fatal error when initializing pipe or selector");
            e.printStackTrace();
        }
        this.output = output;
    }
     ...
     * {SelectableChannel}. A thread is created to read from the
